...
- `s2_tpch_sf100_demo.ipynb` — SingleStore TPC-H SF100 benchmark notebook with optimized schema.
...
**Note**: SingleStore results use optimized schema with shard keys and sort keys (native SingleStore features). PostgreSQL results use standard heap tables.
...
2. Set environment variables:
   ...
   ```
3. Initialize your schema and load SF100 data using the included `load_tpch_sf100_to_postgres.sh` script.
4. Run the benchmark queries:
   ```bash
   ./scripts/run_postgres_queries.sh
   ```
5. (Optional) To capture MV timings:
   ```bash
...
2. Run the setup cells to create optimized tables with shard keys, hash keys, and sort keys.
3. Run the pipeline cells to ingest SF100 data from public S3.
4. Execute the benchmark query cells (Q1, Q4, Q21).

**SingleStore optimization notes**: The benchmark uses native SingleStore features including:
- SHARD KEY for distributed execution
- UNIQUE KEY (hash-based) for fast lookups
- SORT KEY (clustered columnstore) for segment elimination
